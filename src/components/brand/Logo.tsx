...
export const Logo: React.FC<LogoProps> = ({
  ...
  if (iconOnly) {
    return (
      <div className={`relative flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-105 ${className}`}>
        <svg
        ...
        >
          ...
          <path
            fill="url(#aimapping_official_grad)"
  ...
  return (
    <div className="flex items-center select-none group cursor-pointer">
      <img
        src={logoSrc}
        alt={alt}
        className={`h-8 sm:h-9 w-auto object-contain transition-transform duration-300 group-hover:scale-105 ${className}`}
      />
    </div>
  );
};
